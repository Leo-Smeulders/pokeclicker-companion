if (App.game) {
  intervalMap.set(
    "dungeonName",
    document.currentScript.getAttribute("dungeonName")
  );
  intervalMap.set(
    "dungeonOption",
    document.currentScript.getAttribute("dungeonOption")
  );
  intervalMap.set(
    "autodungeonScript",
    new (function () {
      ...
      this.checkIfBossAppeared = function () {
        const index = DungeonRunner.map
          .board()
          [this.floor].findIndex((row) =>
            row.find((tile) => tile.type() === 4 && tile.isVisible === true)
          );
        if (index >= 0) {
          this.boss.x = index;
          this.boss.y = DungeonRunner.map
            .board()
            [this.floor][index].findIndex((tile) => tile.type() === 4);
        }
      };
      ...
      this.allAroundVisited = function (x, y) {
        ...
        const top = this.getBoardTile(nextTopY, nextTopX) || {
          isVisited: true,
        };
        const right = this.getBoardTile(nextRightY, nextRightX) || {
          isVisited: true,
        };
        const bottom = this.getBoardTile(nextBottomY, nextBottomX) || {
          isVisited: true,
        };
        const left = this.getBoardTile(nextLeftY, nextLeftX) || {
          isVisited: true,
        };

        return (
          top.isVisited && right.isVisited && bottom.isVisited && left.isVisited
        );
      };
      ...
      this.loop = async function () {
        while (DungeonRunner.dungeonFinished() === false && this.runs > 0) {
          ...
          if (
            ![-1, this.size].includes(newY) &&
            ![-1, this.size].includes(newX)
          ) {
      ...
      this.launchDungeon = async function () {
        ...
        if (this.runs >= 100) {
          ...
          const timesCleared =
            App.game.statistics.dungeonsCleared[dungeonIndex]();
          this.runs = this.runs - timesCleared;
        }
        ...
        while (
          App.game.wallet.currencies[2]() >=
            (player.town.dungeon ? player.town.dungeon.tokenCost : Infinity) &&
          this.runs > 0
        ) {
