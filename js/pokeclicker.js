...
const purifyID = "autoPurifyScript";
const undoPurifyID = "undoAutoPurifyScript";
...
const purifyURL = `${PURIFY}/${purifyID}.js`;
const undoPurifyURL = `${PURIFY}/${undoPurifyID}.js`;
...
const injectScript = (fileName, id, params) => {
  if (document.getElementById(id))
    return { status: `${id} already loaded`, response: id };
...
const getTogglesStates = () => {
  ...
  const purify = document.getElementById(purifyID);
...
const selectDungeonsOn = (params) =>
  injectScript(dungeonsURL, dungeonsID, params);
/**
...
const toggleAutoPurifyOff = () => {
  ejectScript(purifyID);
  injectScript(undoPurifyURL, undoPurifyID);
  return ejectScript(undoPurifyID);
};
const toggleAutoPurifyOn = (params) =>
  injectScript(purifyURL, purifyID, params);
...
const mapMessageToFunction = {
  ...
  "toggle-purify-on": toggleAutoPurifyOn,
  "toggle-purify-off": toggleAutoPurifyOff,
